...
function findInAgenda(agenda, phone) {
    ...
    addresses.forEach((address) => {
        let phoneFound = [...address.matchAll(/[+]\d{1,2}-\d{3}-\d{3}-\d{3}/g)][0][0];
        if (phoneFound.slice(1).indexOf(phone) !== -1) {
            founds++;
            addressFound.name = address.match(/<.*?>/)[0].slice(1, -1);
            addressFound.address = address.split(phoneFound).join('').split('<')[0].trim();
        }
    });

    if (founds > 1 || founds === 0) {
        addressFound = null;
    }
