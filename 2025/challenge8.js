...
// ignorando mayúsculas y minúsculas al contar, 
// pero devolviendo la letra tal como aparece en el string.
...
function findUniqueToy(toy) {
    ...
    for (const letter of letters) {
        ...
        counts[letterMinus]['count']++;
        counts[letterMinus]['letters'].push(letter);
    }

    const uniqueLetter = Object.values(counts).find(item => item.count === 1)?.letters[0] ?? '';
